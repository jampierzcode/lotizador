$(document).ready(function () {
  ...
  if (user && proyect) {
    ...
    function buscar_target_socials() {
      let funcion = "buscar_by_user_target_socials";
      ...
      $.post(
        ...
        (response) => {
          ...
          if (response.trim() !== "no-register-amenidades") {
            ...
            pintar_amenidades(amenidades);
          } else {
            $("#title-amenidades").addClass("hidden");
            $("#title-amenidades").html("");
            $(".amenidades").html("");
            console.log("sin amenidades aun");
          }
        }
      );
    }
    function pintar_amenidades(amenidades) {
      let template = "";
      amenidades.forEach((a) => {
        template += `
        <div class="w-full flex flex-col items-center gap-4">
          <img class="w-[50px]" src="../imagenes/amenidades/${a.icono}.png" alt="${a.icono}">
          <span class="block text-center w-full">${a.nombre}</span>
        </div>
        `;
      });
      $(".amenidades").html(template);
    }
    // redes sociales
    ...
    function buscar_multimedia() {
      let resultado = fetchMultimediaProyecto(proyect);
      resultado
        .then((response) => {
