$(document).ready(function () {
  ...
  var dataTargetUser;
  var statenewDataSocial;

  var dataTargetSocial;
  var iconsSocials = [
    {
      title: "email",
  ...
  function pintarRedesSociales() {
    const redesSocialesContainer = $("#redesSociales");
    // redesSocialesContainer.html("");
    let template = "";
    statenewDataSocial.forEach((red, index) => {
      const $li = $('<li class="item gap-4"></li>');
      const $details = $('<div class="details w-full"></div>');
      const $contenidoInputs = $('<div class="w-full"></div>');
      const $svg = iconsSocials.find((e) =>
        e.title === red.social ? e.img : ""
      );
      $details.append($svg.img, $contenidoInputs);
      const $span = $(`<span class="text-sm">${red.username}</span>`);
      ...
      $inputUrl.on("keyup", function () {
        ...
        $(`#switch-${index}`).prop("checked", red.status);
        if (
      ...
      let actived =
        red?.status == "0" || red?.status == "1"
          ? Boolean(Number(red?.status))
          : red?.status;
      const $botonEncend = $('<div class="boton-encend"></div>');
      const $switchInput = $(
        `<input type="checkbox" class="btnchecklink" id="switch-${index}">`
      ).prop("checked", actived);
      const $switchLabel = $(
        `<label class="lbl-switch" for="switch-${index}"></label>`
      );

      $switchInput.on("change", function () {
        red.status = $switchInput.is(":checked");
        pintarPreviewRedes(statenewDataSocial);
        console.log(statenewDataSocial);
        console.log(dataTargetSocial);
        if (
          JSON.stringify(statenewDataSocial) !==
          JSON.stringify(dataTargetSocial)
        ) {
      ...
      // template += `${$li}`;
      redesSocialesContainer.append($li);
    });
  ...
  $("#actualizar").on("click", function () {
    ...
    console.log("Redes Sociales Actualizadas:", redesSocialesActualizadas);
  });
  // funcion de app socail icons
  // Función para actualizar el contenedor de usernames
  ...
  $("#email, #facebook, #instagram, #whatsapp, #tiktok ").on(
    "keyup",
    function () {
      const etiqueta = $(this).attr("id");
      switch (etiqueta) {
        case "email":
          statenewDataSocial.name_user = $(this).val();
          break;
        case "facebook":
          statenewDataSocial.job = $(this).val();
          break;
        case "instagram":
          statenewDataSocial.custom_description = $(this).val();
          break;
        case "whatsapp":
          statenewDataSocial.custom_description = $(this).val();
          break;
  ...
  $("#cancelar_submit").click(function () {
    statenewDataSocial = dataTargetSocial;
    $("#redesSociales").html("");
    buscar_target_socials();
  ...
  $("#send_submit").click(function () {
    ...
    if (disabled === "false") {
      ...
      if (createActive) {
        let funcion = "update_social_networks_id";
        let jsonRedesSociales = JSON.stringify(statenewDataSocial);
        $.post(
          "../../controlador/UsuarioController.php",
          { funcion, redes_sociales: jsonRedesSociales },
          (response) => {
            console.log(response);
            if (response.trim() === "update-sucess") {
              alert("Se actualizaron correctamente tus redes sociales");
              disabled_update_acces_btn();
              $("#redesSociales").html("");
              buscar_target_socials();
            } else {
        ...
        let funcion = "update_social_networks";
        let jsonRedesSociales = JSON.stringify(statenewDataSocial);
        $.post(
          "../../controlador/UsuarioController.php",
          { funcion, redes_sociales: jsonRedesSociales },
